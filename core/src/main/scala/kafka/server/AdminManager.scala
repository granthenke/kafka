...
class AdminManager(val config: KafkaConfig,
  ...
  def deleteTopics(timeout: Int,
                   deleteInfo: Map[String, Errors],
                   responseCallback: Map[String, Errors] => Unit) {

    // 1. map over topics calling the asynchronous delete
    val metadata = deleteInfo.map { case (topic, errors) =>
      if (errors == Errors.NONE) {
        try {
          AdminUtils.deleteTopic(zkUtils, topic)
          DeleteTopicMetadata(topic, errors)
        } catch {
    ...
    if (timeout <= 0 || !metadata.exists(_.error == Errors.NONE)) {
      ...
      val delayedDeleteKeys = deleteInfo.keySet.map(new TopicKey(_)).toSeq
      // try to complete the request immediately, otherwise put it into the purgatory
