...
class CreateTopicRequestTest extends BaseRequestTest {
  ...
  def testValidCreateTopicRequests() {
    ...
    validateValidCreateTopicRequests(new CreateTopicRequest(Map("topic1" -> new CreateTopicRequest.TopicDetails(1, 1)).asJava, timeout))
    validateValidCreateTopicRequests(new CreateTopicRequest(Map("topic2" -> new CreateTopicRequest.TopicDetails(1, 3)).asJava, timeout))
    val config3 = Map("min.insync.replicas" -> "2").asJava
    validateValidCreateTopicRequests(new CreateTopicRequest(Map("topic3" -> new CreateTopicRequest.TopicDetails(5, 2, config3)).asJava, timeout))
    // Manual assignments
    val assigments4 = replicaAssignmentToJava(Map(0 -> List(0)))
    validateValidCreateTopicRequests(new CreateTopicRequest(Map("topic4" -> new CreateTopicRequest.TopicDetails(assigments4)).asJava, timeout))
    val assigments5 = replicaAssignmentToJava(Map(0 -> List(0, 1), 1 -> List(1, 0), 2 -> List(1, 2)))
    val config5 = Map("min.insync.replicas" -> "2").asJava
    validateValidCreateTopicRequests(new CreateTopicRequest(Map("topic5" -> new CreateTopicRequest.TopicDetails(assigments5, config5)).asJava, timeout))
    // Mixed
    ...
    validateValidCreateTopicRequests(new CreateTopicRequest(Map(
      ...
      "topic8" -> new CreateTopicRequest.TopicDetails(assigments8)).asJava, timeout)
    )
  }
  ...
  def testInvalidCreateTopicRequests() {
    ...
    // Basic
    validateInvalidCreateTopicRequests(new CreateTopicRequest(Map(existingTopic -> new CreateTopicRequest.TopicDetails(1, 1)).asJava, timeout),
      Map(existingTopic -> Errors.TOPIC_ALREADY_EXISTS))
    validateInvalidCreateTopicRequests(new CreateTopicRequest(Map("invalid-partitions" -> new CreateTopicRequest.TopicDetails(-1, 1)).asJava, timeout),
      Map("invalid-partitions" -> Errors.INVALID_PARTITIONS))
    validateInvalidCreateTopicRequests(new CreateTopicRequest(Map("invalid-replication" -> new CreateTopicRequest.TopicDetails(1, numBrokers + 1)).asJava, timeout),
      Map("invalid-replication" -> Errors.INVALID_REPLICATION_FACTOR))
    val invalidConfig = Map("not.a.property" -> "invalid").asJava
    validateInvalidCreateTopicRequests(new CreateTopicRequest(Map("invalid-config" -> new CreateTopicRequest.TopicDetails(1, 1, invalidConfig)).asJava, timeout),
      Map("invalid-config" -> Errors.INVALID_CONFIG))
    val invalidAssignments = replicaAssignmentToJava(Map(0 -> List(0, 1), 1 -> List(0)))
    validateInvalidCreateTopicRequests(new CreateTopicRequest(Map("invalid-assignment" -> new CreateTopicRequest.TopicDetails(invalidAssignments)).asJava, timeout),
    ...
    validateInvalidCreateTopicRequests(
      new CreateTopicRequest(Map(
        ...
        "partial-none" -> new CreateTopicRequest.TopicDetails(1, 1)).asJava, timeout),
      Map(
    ...
    // Timeout
    validateInvalidCreateTopicRequests(new CreateTopicRequest(Map("invalid-timeout" -> new CreateTopicRequest.TopicDetails(10, 3)).asJava, 1),
      Map("invalid-timeout" -> Errors.REQUEST_TIMED_OUT))
    // The topic should still get created eventually
    TestUtils.waitUntilMetadataIsPropagated(servers, "invalid-timeout", 0)
    validateTopicExists("invalid-timeout")
  }
